...
#--srcTbl = 'sbx2_1_croldv85_remisprd_edb_transplant_periods'
#--srcTbl1 = 'ado1-eqrs-sbx2-1-patient-eqrs_ptnt_ptnt'
#--trgtDb = 'ado1-eqrs-sbx2-1-data-migration-database'
#--trgtTbl = 'eqrs_eqrs_cvrg_calc_ptnt_trnsplnt_cvrg_prd'
...
ptnt_trnsplnt_cvrg_prd = glueContext.create_dynamic_frame.from_catalog(
    database = args['srcDb'], 
    table_name = args['srcTbl'], 
    transformation_ctx = "ptnt_trnsplnt_cvrg_prd")

#ptnt_trnsplnt_cvrg_prd.printSchema()
#print ("Count:", ptnt_trnsplnt_cvrg_prd.count() )
ptnt_trnsplnt_cvrg_prd_df = ptnt_trnsplnt_cvrg_prd.toDF() #.printSchema()
...
#inner join#
resultDf = ptnt_trnsplnt_cvrg_prd_df.join(ptnt_df, ptnt_trnsplnt_cvrg_prd_df.PAT_ID == ptnt_df.remis_ptnt_id, "inner")


resultDf = resultDf \
.select(col("ETP_ID").alias("PTNT_TRNSPLNT_CVRG_PRD_ID")
            ,col("PTNT_ID").alias("PTNT_ID")
            ,col("EDB_BENE_ESRD_TRNSPLNT_STRT_DT").alias("PTNT_TRNSPLNT_STRT_DT")
            ,col("EDB_BENE_ESRD_TRNSPLNT_STOP_DT").alias("PTNT_TRNSPLNT_STOP_DT")
            ,col("TX_FAIL_CODE").alias("TRNSPLNT_FAILD_CD")
