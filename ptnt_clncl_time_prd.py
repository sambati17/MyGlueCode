    ...
    def get_placeholder_data():
        ...
        table_data = [
            (
                ...
                'prodpreview2-1_eqrs_crown_clinical_month_year',
            ),
            (
                'EQRS',
                'PTNT_CLNCL_TIME_PRD',      
                'prodpreview2-1',
                'ado1-eqrs-prodpreview2-1-data-migration-database',
                'prodpreview2-1-clinical-eqrs_ptnt_clncl_data_ptnt_clncl_time_prd',
            ),
        ]
...
class GlueTransformer(GlueTable):
    ...
    def write_to_parquet(self, parquet_location):
...
class MergeRuleExtractor:
    ...
    def col_expr_builder(self) -> Tuple[List[str], List[TableSpec], List[TableSpec]]:
        ...
        # do_rows={5,6,7,8,9,10,11}    
        for index, row in self.merge_rules.iterrows():
          #  if index not in do_rows:
           #     break
...
def test():
    target_db = 'EQRS'
    target_table = 'CLINICAL_MONTH_YEAR'
    env = 'sbx2-1'
